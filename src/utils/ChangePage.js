...
export class WantBuyList extends React.Component {
    ...
    WantBuyList = () => {
        ...
        return data.map((item, index) => {
            ...
            return <View key={item.value.id}>
                <WingBlank size="lg">
                    <WhiteSpace size='lg'/>
                    <Card>
                        <TouchableNativeFeedback
                            disabled={this.state.waiting}
                            onPress={() => this.props.type === 'MyUser' ? this.UpdateWantBuy(item) : ''}
                        >
                        <Card.Header
                            ...
                            extra={
                                <View style={{alignItems: 'flex-end'}}>
                                    <Text style={{color: 'red'}}>
                                        ...
                                        <Text
                                            style={{fontSize: 18}}>{item.value.minPrice + '~~' + item.value.maxPrice}</Text>
                                    </Text>
                                </View>}
                        />
                        </TouchableNativeFeedback>
                        <Card.Body style={{flexDirection: 'column'}}>
                            ...
                            <Text style={{marginLeft: 10, marginTop: 10}}>{item.value.intro}</Text>
                        </Card.Body>
                        <Card.Footer
