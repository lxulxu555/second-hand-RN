...
class UpdateUser extends Component {
  ...
  _UploadImage = async image => {
    ...
    fetch('http://101.201.151.118:8800/api/upload/image', {
      method: 'POST',
      headers: {
    ...
    return (
      <View style={{flex: 1, alignItems: 'center'}}>
        <List style={{marginTop: 10, width: '95%'}}>
          ...
          <List.Item
            disabled
            extra={<Text>{UserData.username}</Text>}
            arrow="horizontal">
            <TouchableOpacity
              onPress={() =>
                UserData.flag === null
                  ? this.props.navigation.push('Nickname', {
          ...
          <List.Item
            disabled
            arrow="horizontal"
            extra={<Text>{UserData.nickname}</Text>}>
            <TouchableOpacity
              onPress={() =>
                this.props.navigation.push('Nickname', {
          ...
          <List.Item
            disabled
            arrow="horizontal"
            extra={<Text>{UserData.phone}</Text>}>
            <TouchableOpacity
              onPress={() =>
                this.props.navigation.push('Nickname', {
          ...
          <List.Item
            disabled
            arrow="horizontal"
            extra={<Text>{UserData.email}</Text>}>
            <TouchableOpacity
              onPress={() =>
                this.props.navigation.push('Nickname', {
          ...
          <List.Item
            disabled
            arrow="horizontal"
            extra={<Text>{UserData.sex}</Text>}>
            <TouchableOpacity onPress={() => this.setState({SexVisible: true})}>
          ...
          <List.Item
            disabled
            arrow="horizontal"
            extra={<Text>{UserData.intro}</Text>}>
            <TouchableOpacity
              onPress={() =>
                this.props.navigation.push('Nickname', {
