...
import {reqAllClass, reqAddProduct, reqUpdateProduct} from '../../api'
...
class SendProduct extends Component {
    ...
    saveProduct = async () => {
        ...
        if (type === 'setting') {
            ...
            product.classify2_id = this.state.classId
        }
        product.intro = this.state.productText
        product.price1 = this.state.price
        product.weixin = this.state.weixin
        if (product.name === '' && product.price1 === '' && product.weixin === '' && product.intro === '') {
            Toast.fail('不能为空')
        } else {
            const result = type === 'setting' ? await reqUpdateProduct(product) : await reqAddProduct(product)
            if (result.code === -1) {
    ...
    SettingProduct = () => {
        ...
        this.setState({
            ...
            productText: detail.intro,
            title: detail.name,
            weixin: detail.weixin,
            price: detail.price1.toString()
        })
    }

    componentWillMount() {
        BackHandler.addEventListener('hardwareBackPress', this.onBackAndroid)
        if (this.props.navigation.getParam('type') === 'setting') {
            this.SettingProduct()
        }
    }
        ...
        return (
            <View style={{backgroundColor: '#FFFFFF', flex: 1}}>
                <TextInput
                    ...
                    placeholder='品牌，型号，几成新，入手渠道，转手原因...'
                    style={{paddingLeft: 20, fontSize: 15, height: '22%', textAlignVertical: 'top'}}
                    onChangeText={(text) => this.setState({
                        productText: text
                    })}
                />
                <ImageWall ref={ref => this.ImageWall = ref} ImageUrl={this.state.ImageUrl} site='/deal/goods'/>
                <View style={{alignItems: 'center'}}>
                    <List style={{width: '85%', paddingTop: 20}}>
                        <InputItem
                            ...
                            extra={<Text style={{fontSize: 13, color: '#778899'}}>吸引顾客</Text>}
                        ...
                        <InputItem
                            ...
                            style={{paddingLeft: 15}}
                            extra={<Text style={{fontSize: 13, color: '#778899'}}>开个价</Text>}
                        ...
                        {
                            this.props.navigation.getParam('type') === 'setting' ? <View/> :
                                <List.Item disabled arrow="horizontal" onPress={() => this.showDemoModal()}>
                                    <Text style={{
                                        margin: 7,
                                        fontSize: 15
                                    }}>{this.state.className !== '' ? this.state.className + '/' + this.state.twoClassName :
                                        <Text>请选择商品分类</Text>}</Text>
                                </List.Item>
                        }
